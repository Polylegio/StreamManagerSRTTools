...
struct UriParserInit
{
    UriParserInit()
    {
        ...
        types["rist"] = UriParser::RIST;
        types["https"] = UriParser::HTTPS;
        types["relay"] = UriParser::RELAY;
        types["ndi"] = UriParser::NDI;
        types["tudp"] = UriParser::TUDP;
        types["zixi"] = UriParser::ZIXI;
        types["analyzer"] = UriParser::ANALYZER;
        types[""] = UriParser::UNKNOWN;
    }
} g_uriparser_init;

UriParser::UriParser(const std::string& strUrl, DefaultExpect exp)
{
    Parse(strUrl, exp);
}
...
std::string UriParser::proto(void) const
{
    return m_proto;
}
...
std::string UriParser::host(void) const
{
    return m_host;
}

std::string UriParser::port(void) const
{
    return m_port;
}
...
std::string UriParser::path(void) const
{
    return m_path;
}

std::string UriParser::queryValue(const std::string& strKey) const
{
    return m_mapQuery.at(strKey);
}

void UriParser::Parse(const std::string& strUrl, DefaultExpect exp)
{
    int iQueryStart = -1;
    ...
    {
        std::string realhost = m_host.substr(atp+1);
        std::string prehost;
        if ( atp > 0 )
        {
            ...
            {
                std::string pw = prehost.substr(colon+1);
                std::string user;
                if ( colon > 0 )
